...
def dfs(n, maze, start, end):
    ...
    while pos != end:        
        ...
        if(check_pos(row,col,n,maze)==True  and pos+n<=end and pos+n  not in visited and maze[row+1][col]==0):
            cost+=3
            #print('down true')
            stack.append(pos+n)
            visited.append(pos+n)
            print('down',stack)
            pos = pos+n
        elif(check_pos(row,col,n,maze)==True  and pos+1<=end and pos+1 not in visited and col+1<n and maze[row][col+1]==0):
            cost+=3
            stack.append(pos+1)
            visited.append(pos+1)
            print('right',stack)
            pos = pos+1
        elif(check_pos(row,col,n,maze)==True and pos-n>=start and pos-n not in visited and maze[row-1][col]==0):
            cost+=3
            stack.append(pos-n)
            visited.append(pos-n)
            print('up',stack)
            pos = pos-n
        elif(check_pos(row,col,n,maze)==True and pos-1>=start and pos-1 not in visited and col-1>=0 and maze[row][col-1]==0):
            cost+=3
            stack.append(pos-1)
            visited.append(pos-1)
            print('left',stack)
...
def vcs(n,maze,start ,end):
    ...
    visited.append(start)
    while pos != end:
        x = sorted(cost.items(),key=lambda x: x[1] )
        q = x[0][0]
        pos=q
        cost.pop(q)
        row = q//n
...
def astar(n,maze,start,end):
    ...
    while pos != end:
        ...
        if(check_pos(row,col,n,maze)==True  and q+n<=end and q+n  not in visited and maze[row+1][col]==0):
            c+=3
            heuristic = abs((row+1)-d_row) + abs((col)-d_col)
            #print(heuristic)
            cost[q+n]=3+heuristic
            visited.append(q+n)
            print('down')
        if(check_pos(row,col,n,maze)==True  and q+1<=end and q+1 not in visited and col+1<n and maze[row][col+1]==0):
            c+=2
            heuristic = abs((row)-d_row) + abs((col+1)-d_col)
            cost[q+1]=2+heuristic
            visited.append(q+1)
            print('right')
        if(check_pos(row,col,n,maze)==True and q-n>=start and q-n not in visited and maze[row-1][col]==0):
            c+=2
            heuristic = abs((row-1)-d_row) + abs((col)-d_col)
            cost[q-n]=2+heuristic
            visited.append(q-n)
            print('up')
...
def search_algo(choice,n, maze, start, end):
    print('x:',choice)
    if(choice == '1'):
        bfs(n, maze, start, end)
    elif(choice == '2'):
        print('dfs')
        dfs(n, maze, start, end)
    elif(choice == '3'):
        vcs(n, maze, start, end)
    elif(choice == '4'):
        astar(n, maze, start, end)
    else:
        return 0 
...
if __name__ == "__main__":
    ...
    n=int(n)
    print('Enter 1 to reach meat using BFS \n 2 to use DFS \n 3 to use VCS and \n 4 to use A*')
    choice = input(':')
    print("u :",choice)
    start, end = startend_postion(n)
    randno = randomize(n)
    maze = prepare_maze(n, randno, start, end)
    search_algo(choice,n, maze, start, end)
